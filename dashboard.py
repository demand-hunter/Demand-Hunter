...
# Chave exclusiva para comunicação entre o Persona e o Demand Hunter.
PERSONA_API_KEY = os.environ.get(
    "PERSONA_API_KEY",
    "",
)
...
# Três liberações diárias: 08h, 12h e 19h,
...
def acesso_valido():
    return (
        request.cookies.get("demand_access")
        == token_atual()
    )
...
def protegido(func):
    ...
    def wrapper(*args, **kwargs):
        if not acesso_valido():
            return jsonify({
                "erro": (
                    "Acesso vencido. Consulte o Canal "
                    "Oferta Certa para obter o novo link."
                )
            }), 403
...
# =========================================================
# SEGURANÇA DA API DO PERSONA
# =========================================================

def persona_api_protegida(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        ...
        if not PERSONA_API_KEY:
            return jsonify({
                "ok": False,
                "erro": (
                    "PERSONA_API_KEY não foi configurada "
                    "no servidor."
                ),
            }), 503
...
def home():
    ...
    <body>
        <div class="box">
            ...
            <p>
                Esta rodada de ofertas não está
                disponível por este endereço.
            </p>

            <p class="aviso">
                Use o link mais recente publicado
                no Canal Oferta Certa.
            </p>

            <p>
                Novas rodadas: 08h, 12h e 19h.
            </p>
        </div>
    </body>
    </html>
    ...
    if not hmac.compare_digest(
        ...
        <body>
            <div class="box">
                ...
                <p>
                    Consulte o Canal Oferta Certa
                    para acessar a rodada mais recente.
                </p>
...
def admin_link():
    ...
    link = (
        f"{base}/acesso/{token_atual()}"
    )

    mensagem = (
        "🔥 NOVA RODADA DE OFERTAS DISPONÍVEL!\n\n"
        f"👉 Acesse agora: {link}\n\n"
        "⏳ Este link será substituído "
        "na próxima rodada.\n"
...
# =========================================================
# API PARA INTEGRAÇÃO COM O PERSONA
# =========================================================
...
def api_status():
    ...
    return jsonify({
        ...
        "status": "online",
        "periodo": periodo_atual(),
        "quantidade_oportunidades": len(
            oportunidades
        ),
        "arquivo_disponivel": os.path.exists(
            os.path.join(
                BASE_DIR,
                "oportunidades.csv",
            )
...
@app.route("/api/v1/oportunidades")
@persona_api_protegida
def api_oportunidades():
    try:
        oportunidades = carregar_oportunidades()

        limite_texto = request.args.get(
            "limite",
            "",
        ).strip()
...
def api_resumo():
    try:
        ...
        oportunidades_com_score = sorted(
            oportunidades,
